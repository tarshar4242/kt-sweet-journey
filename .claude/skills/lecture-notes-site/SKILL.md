...
description: 把 Google Drive 的課程資料夾（講義簡報＋錄影回放）做成「逐針詳細課堂筆記 HTML」＋「akira 風格多層次互動學習地圖 HTML」。當使用者貼出 Drive 課程資料夾連結、提到晨學講堂 EP 集數、或要求「課堂筆記／學習地圖／逐針記錄」時使用。
---
...
- 影片抽格：`ffmpeg`（在 /opt/pw-browsers/ffmpeg-*）`-vf "fps=1/15,scale=1280:-2"` 出 jpg，
  用 Read 工具**親眼逐格看**並記錄每頁簡報／示範畫面（這就是「一針一針」）。
...
2. commit（繁中訊息）＋ push 到指定分支；**未經同意不碰 main、不開 PR**。
3. 兩檔各發佈 Artifact（favicon 固定：地圖🧠、筆記📝；重發用同檔名同路徑保 URL）。
4. 回覆使用者：Artifact 連結＋raw.githack 備用連結
   （`https://raw.githack.com/<owner>/<repo>/<branch>/<file>`，Drive iframe 在此可正常內嵌）
   ＋說明「合併到 main 後 GitHub Pages 正式網址生效」。
