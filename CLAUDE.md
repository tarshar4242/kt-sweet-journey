...
⚠️ **日誌以「專案」命名，不以某一件產出物命名**（例：專案是「AI 開機日」，護照只是其中一件教具，
   日誌就叫 `AI協作日誌02-AI開機日.md`，不叫護照）。
2. **無論用哪個技能，動手前一律先讀 `docs/` 裡既有的日誌當範本**：
   `docs/小熊健康小屋-AI協作日誌.html`（01）、`docs/AI協作日誌02-AI開機日.md`（02，依正式技能產出）。
...
- **逐幀筆記做完要交付到 Drive（2026-08-20 使用者定案，正本在 SKILL.md 第 4 步第 5 點）**：
  上架網站**不等於**交付完成。兩份 HTML 還要**轉成 PDF**放進她指定的課程 Drive 資料夾
  （訂閱制第三季＝「第三季」資料夾 `1LSPX3vXbJ3oCscttTpsQacClbcSNL1f4`）。
  轉 PDF 前必須注入 CSS 把互動內容攤平（否則只印得到首頁），詳細做法見 SKILL.md。
  ⚠️ Drive MCP `create_file` 傳不動幾 MB 的 PDF、且會跳核准，實務走「SendUserFile 傳給她 → 她拖進資料夾」。
- **逐幀筆記密度規則（2026-08-19 使用者定案，A＋B 兩級都設，正本在 SKILL.md）**：
  「逐幀看多細」與「輸出寫多長」是兩件事，分開控制——
  **A 抽格密度**依影片總長自動選，把總格數壓在 ~200（≤50 分→每15秒；50–90 分→每25秒；
  >90 分→每35秒、>150 分→每40秒或先問是否分集）；這是製作時間／token 的真正槓桿。
