...
一律走 `.claude/skills/ai-collab-log/SKILL.md`，**動手前先讀 `docs/` 裡既有的那幾份日誌當範本**。
格式固定：單檔 HTML、第一人稱她的口吻、封面＋內容目錄＋導讀四節（為什麼做／協作主線／各方角色／資料界線）
＋九部（完整紀錄×2、偏航與修正、決策帳、關鍵字辭典、研究素材、文件狀態、仍待確認、下一步），存到 `docs/`。
⚠️ **這條的由來：她說「AI協作日誌」時，Claude 已經有好幾次沒去查既有格式就自創了索引式交接文件（檔案清單＋
定案總表＋時間軸），交出錯誤產物。那是 `project-handover-doc` 的東西，不是協作日誌。**
